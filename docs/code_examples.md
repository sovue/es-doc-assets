...
:::warning
**ВНИМАНИЕ** Для того чтобы игра распознала сочетания клавиш, нужно переключиться на **латинскую раскладку клавиатуры**.
...
:::warning
**ВНИМАНИЕ** Поддерживается объявление только "цельных" спрайтов.

"Цельным" спрайтом называется спрайт, у которого тело, одежда, эмоция и прочее идут одним изображением, а не каждая часть спрайта отдельно, как в БЛ.
...
Эти два варианта уже объявлены в игре глобально и могут использоваться как [изображение](https://www.renpy.org/doc/html/displaying_images.html#image), объявленное с помощью `Image Statement`.
...
:::tip
**ПОДСКАЗКА** Данный способ объявления работает в `init python`. Для обычного `init` используйте:
...
:::tip
**ПОДСКАЗКА** Если вы хотите добавить отображение играемой музыки в экран, то можно сделать так:
...
`store.map_zones_mymod` содержит в себе список зон, при наведении на координаты которых `idle`-версия будет сменяться на `hover`.
...
- `show_map_mymod()` - перебрасывает на лейбл, показывающий карту. Проще говоря, показывает карту.
- `init_map_zones_realization_mymod()` - инициализирует карту.


:::danger
**ВАЖНО** Инициализация карты должна происходить один раз за весь мод.
...
- `screen_map_condition`<`List`> - список, состоящий из False. Количество False в списке определяет количество объектов, которые могут быть на карте.
...
:::tip
**ПОДСКАЗКА** Стоит напомнить, что название переменных лучше придумывать более уникальными, чтобы избежать конфликтов с другими модификациями.
...
Функция `screens_map_reset_condition` сбрасывает переменные, связанные с работой карты. А `screens_map_set_condition` принимает два аргумента `label`<`String`> и `count`<`Int`>. Устанавливает лейбл, к которому мы должны перейти после карты, и количество локаций.

Перейдём к написанию самой карты. Она будет представлять собой `screen`, принимающий в качестве аргумента словарь.
...
:::tip
**ПОДСКАЗКА** Можно обойтись и без лейбла `my_mod_true_exit`: вы можете попробовать добавить к вашей кнопке выхода в главном меню следующее действие:
...
Настраиваем наш экземпляр `modGallery`: задаём изображение заблокированного (ещё не открытого) варианта картинки и отключаем навигацию.
...
Затем создаём словари, что будут содержать в себе иллюстрации и фоны, а потом заполняем с помощью цикла нашу галерею.
...
**ПОДСКАЗКА** Если необходимо, можно создать специальную функцию, что позволяет нам открыть все изображения из нашей галереи.
...
Финальные штрихи — создаём кнопки для навигации между страницами галереи, также ставим текст, что показывает текущую страницу и их общее количество.
...
:::tip
**ПОДСКАЗКА** Существует возможность добавить перевод названия мода и персонажей на:
...
Затем внутри значения с нашим именем создаём ещё два: одно для русского перевода, второе — для английского.
...
Затем создаём сами тэги для перевода с помощью функций, что будут возвращать нужный вариант текста в зависимости от значения переменной `persistent.translate_text_lang`. Если значение `ru`, то показывает русский вариант текста, если `en`, то английский. Внутрь тэгов будем записывать русский и английский вариант текста.
...
:::tip
**ПОДСКАЗКА** Как вариант, можно создать кнопку в меню мода, что будет переключать язык повествования.
...
Движения хранятся в папке `motions`, эмоции - в папке `expressions`. Названия движений и эмоций берутся из файлов Live2D, затем приводятся к нижнему регистру, и если они начинаются с имени спрайта, за которым следует подчёркивание, то этот префикс удаляется.
...
Ren'Py поддерживает плавную смену анимации при работе с Live2D. Обычно, когда Ren'Py переходит от одной анимации к другой, переход происходит резко: одна анимация останавливается, а другая запускается.
...
:::danger
**ВАЖНО** Поддерживаются не все модули!
...
:::warning
**ВНИМАНИЕ** Если установлены сторонние модификации, то в списке будут и персонажи из модов.
