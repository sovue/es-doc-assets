...
Часто в описаниях модов в мастерской можно увидеть: *"Используйте [Менеджер Модов](https://steamcommunity.com/sharedfiles/filedetails/?id=1826799366&searchtext=mod+manager) для устранения конфликтов."* Или ещё лучше: *"Удалите все модификации перед загрузкой этой."*

А проблема следующая — **некоторые [переменные](/guide/advanced.md#%D0%BF%D0%B5%D1%80%D0%B5%D0%BC%D0%B5%D0%BD%D0%BD%D1%8B%D0%B5) проинициализированы по два или более раза (например, два мода используют одну и ту же переменную), из-за чего в конечном итоге их значение отличается от того, на которое рассчитывал автор или кодер мода.**
...
Для того чтобы в игре говорил ОченьДлинноеИмяПеременной, каждый раз нужно прописывать переменную его персонажа - `veryLongCharacter`, что совсем неудобно делать много раз. Именно поэтому мододелы используют короткие имена переменных. Пример:
...
Но количество таких имён ограничено: всего 676 двухбуквенных имён и 17 576 трёхбуквенных. Да, может, это и много, но часто люди не думают о других модах и создают свои переменные, которые в итоге перезаписывают другие переменные или сами становятся перезаписанными. Было дело, вместо имени персонажа был указан путь к одному звуку! К сожалению, мы не сохранили снимок этой аномалии, но это более чем возможно воссоздать, если вместо переменной с персонажем использовать переменную с путём к звуку.
...
Допустим, у вас есть переменная для звука выстрела, с названием `shot`. Так её очень легко перезаписать на другой звук или абсолютно другое значение. А теперь добавим префиксы, и у нас получится переменная с названием `rvp_sfx_shot`. Из названия сразу понятно, какому моду она принадлежит (`rvp` - РВП - Рай В Панельке) и каково её назначение (`sfx` - Sound Effects - звуковые эффекты).
...
- `mus` — Музыка
- `img` — Изображения (при использовании с [BG](/guide/#%D0%B1%D1%8D%D0%BA%D0%B3%D1%80%D0%B0%D1%83%D0%BD%D0%B4%D1%8B-%D0%B1%D0%B3) или [CG](/guide/#%D0%B8%D0%BB%D0%BB%D1%8E%D1%81%D1%82%D1%80%D0%B0%D1%86%D0%B8%D0%B8-%D1%86%D0%B3) можно опустить, но тогда желательно, чтобы переменная выглядела так: `bg имяМода_имяПеременной` или `cg имяМода_имяПеременной`)
...
Чтобы не было конфликтов лейблов и функций, их лучше тоже называть, используя эти рекомендации:
...
Ещё один способ инициализации переменных - когда за вас это делает компьютер. Это очень эффективно, это быстрее, чем задавать много переменных вручную, но, с другой стороны, это очень сложно, так как нужно иметь познания в языке программирования Python.
...
Не самый популярный, но довольно эффективный способ - это **постфиксы**. Рекомендации те же, что и для префиксов, но с одним огромным отличием. Постфиксы пишутся **после названия переменной**, из-за чего текстовый редактор может вовремя подсказать правильную переменную.
...
Если папка мода находится по такому пути: `…\steamapps\workshop\content\331470\123456\ray_v_panelke`, то сама папка и всё её содержимое будут перемещены в папку `game` - `…\steamapps\common\Everlasting Summer\game\ray_v_panelke`.

Они будут защищены от перезаписи, их не придётся называть уникальными именами, а пути к файлам будут максимально чистыми.

Также некоторые мододелы думают, что папка мода при публикации обязательно должна находиться в папке `mods`, но **это не так**. Это ещё одна ненужная папка, которая делает пути к файлам длиннее.

### Работа префиксов на практике

#### Расставляем префиксы в начале разработки

Если вы ещё не создали мод, всё проще. Пишите префиксы во время создания переменных.
...
В большей части текстовых редакторов (Notepad++, VS Code, Atom), если начать писать название переменной в сюжете, редактор сам подскажет, какую переменную вставить или дописать.

#### Расставляем префиксы в имеющемся моде

Нужно не только ставить префиксы на все объявленные переменные, но и проходить по коду, заменяя все старые названия на новые.

Советуем использовать команду замены (Ctrl+H в Notepad++, Ctrl+F в VS Code).

При замене рекомендуется выделять переменную с пробелами слева и справа, чтобы затем заменить её на переменную с префиксом с пробелами слева и справа. Пробелы нужны, чтобы при замене редактор не выделял сочетания букв, идентичные переменным, в других словах. Например, если есть переменные `mh` (Михаил) и `mh4` (Михалыч).


 ![](/api/attachments.redirect?id=83361d5b-632e-41ba-b87c-07ab54b08263 "aspect=1")
...
- [Автоинициализация переменных](/guide/code-examples#%D0%B0%D0%B2%D1%82%D0%BE%D0%BC%D0%B0%D1%82%D0%B8%D1%87%D0%B5%D1%81%D0%BA%D0%BE%D0%B5-%D0%BE%D0%B1%D1%8A%D1%8F%D0%B2%D0%BB%D0%B5%D0%BD%D0%B8%D0%B5-%D1%84%D0%B0%D0%B8%D0%BB%D0%BE%D0%B2)
- [Автоинициализация персонажей](/guide/code-examples#%D0%B0%D0%B2%D1%82%D0%BE%D0%BC%D0%B0%D1%82%D0%B8%D1%87%D0%B5%D1%81%D0%BA%D0%BE%D0%B5-%D0%BE%D0%B1%D1%8A%D1%8F%D0%B2%D0%BB%D0%B5%D0%BD%D0%B8%D0%B5-%D0%BF%D0%B5%D1%80%D1%81%D0%BE%D0%BD%D0%B0%D0%B6%D0%B5%D0%B8-%D0%B8-%D0%B8%D0%BD%D1%82%D0%B5%D1%80%D0%B5%D1%81%D0%BD%D1%8B%D0%B5-%D0%BF%D0%BB%D1%8E%D1%88%D0%BA%D0%B8)
...
## *Загрузил мод в Steam, а он выдаёт Traceback. Мод в папке* `mods`

**Q**: Я загрузил в Steam мод, который до этого редактировался и всячески видоизменялся в папке `mods`. Загрузил, основной папкой указал `mods`. После загрузки версии мода из Steam и захода в игру выдаёт трейсбек.

**A**: У вас ошибка в путях к моду при загрузке оного в Steam. Если мод и пути файлов для мода проходят через папку `mods`, то необходимо сделать следующее:
...
**A**: Как бы смешно это ни звучало, но советуем ещё раз перепроверить путь. Поискать ошибки в написании названия и расширения файла, а также поискать лже-буквы (скажем, одна буква, написанная кириллицей внутри слова, написанного латиницей). В крайнем случае — проверьте файл на "битость". Если файл оказался "битым", то Ren'Py его "не увидит".

## *Не понимаю, как загрузить мод из любой директории!*

**Q**: Переместил папку с модификацией на рабочий стол. Поработал над кодом, добавил пару файлов. После загрузки мода в Steam и скачивания его из мастерской вылетает трейсбек. Указывает на ошибку в путях к файлам.
