...
Что нам делать, если мы хотим, чтобы какие-то возможности появлялись у игрока в зависимости от его выборов? Для этого-то мы и использовали переменные! Сейчас же мы воспользуемся специальной конструкцией:
...
```renpy
label my_mod:
    mt "Семён, сегодня после обеда ты будешь убираться в столовой."
...
- возмутиться сложившейся ситуацией;
- попросить её отсрочить дежурство, если мы уже заслужили доверие вожатой;
- отправить её куда подальше, если не помогали.

И в каждом случае нам, как создателям мода, необходимо будет прописать, что произойдёт.

Проверка переменной на значение `True` или `False` происходит схоже:
...
Также переменным можно присваивать не только очки и состояния `True` или `False`, но и всё, что вашей душе заблагорассудится. Приведём пример:
...
Также советуем запомнить, что `True` и `"True"` при проверке переменной — это два разных значения.
