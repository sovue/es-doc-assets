...
В Ren'Py повествованием назвается процесс 

## Метки
**Метка** (в дальнейшем **лейбл**, от англ. *Label*) — это специальная метка, которая позволяет игре перемещаться в конкретные места в коде. Перемещаться на них можно командами Ren'Py `jump` и `call`, различными методами Python и действиями экранов.
...
:::warning
Во избежание конфликтов имя каждой метки должно быть уникальной, отличаться от всех остальных. Лучше всего, когда лейбл имеет 
