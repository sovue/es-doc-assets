...
Рассмотрим один из самых простых методов шифрования кода. Для удаления исходного кода, но сохранения функциональности достаточно удалить `.rpy`-файлы, оставив лишь их `.rpyc`-версии. В `.rpyc`-файле содержится скомпилированный вариант кода вашего мода с нечитаемым для человека синтаксисом. Удаление `.rpy` на работоспособность мода не влияет.
...
:::warning
Необходимо установить `Python` версии не ниже `3.1`, при установке выберите опцию `Добавить Python в PATH`.
...
:::tip
Если папок несколько, то разделяйте их пробелами, если нужно заархивировать лишь один файл, то не забудьте указать его расширение.
...
1. Установите Ren'Py удобным для вас способом, запустите `renpy.exe`.
2. Создайте новый проект в Ren'Py, дайте любое имя, любые настройки.
...
При необходимости вместо всех файлов можно указать конкретные расширения:
...
3. По окончании нажмите любую клавишу для выхода или введите `1` в командную строку, чтобы вернуться.
...
:::warning
1. Необходимо установить `Python` версии не ниже `3.1`, при установке выберите опцию `Добавить Python в PATH`.
2. Поддерживается версия Windows не ниже Windows 8.
